...
注意，简洁语法的属性名总是字符串，这会导致一些看上去比较奇怪的结果。
...
上面代码的方法一直是直接用标识符作为属性名，方法二是用表达式作为属性名，这时要将表达式方在方括号之内。
...
上面代码中，[ keyA ] 和 [ keyB ] 得到的都是 [object object]，所以 [ keyB ] 和 [ keyA ] 覆盖掉，而 myObject 最后只有一个 [ object object ] 属性。
...
如果对象的方法使用了取值函数 (getter) 和 存值函数 (setter)， 则 name 属性不是在该方法上面，而是在该方法的属性的描述对象的 get 和 set 属性上面，返回值是方法名前加上 get 和 set。
...
实际上，引入"可枚举" (enumerable) 这个概念的最初目的，就是让某些属性可以规避掉 for...in  操作，不然所有内部属性和方法都会遍历到。
...
总的来说，操作中引入继承的属性会让问题复杂化，大多数时候，我们只关心对象滋生的属性。所以，尽量不要用 for...in 循环，而用 Object.keys() 代替。
...
Reflect.ownKeys 返回一个数组，包含对象自身的所有键名，不管键名是 Symbol 或 字符串，也不管是否可枚举。
...
上面代码中，变量 z 是 解构赋值所在的对象。
它获取等号右边的所有尚未读取的键 （a 和 b），将它们连同值一起拷贝过来。
...
上面代码中， x 是解构赋值所在的对象，拷贝了对象 obj 的 a 属性。
...
``` js
const o = Object.create({x: 1, y: 2}); // 继承的属性
o.z = 3;
...
上面代码中，变量 x 是单纯的解构赋值，所以可以读取对象 o 继承的属性。
