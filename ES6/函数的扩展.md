...
上面代码中，函数 fetch 没有第二个参数，函数参数的默认值就会生效，然后才是解构赋值的默认值生效，变量 method 才会取到默认值 GET。
...
但是，箭头函数导致 this 总是指向函数定义生效时所在的对象（本例是{id: 42}）， 所在输出的是 42。
