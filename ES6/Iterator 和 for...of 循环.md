...
对数组和 Set 解构进行解构赋值时，会默认调用 Symbol.iterator 方法。
...
实际上，这提供了一种简便机制，可以将任何部署了 Iterator 接口的数据解构，转为数组。也就是说，只要某个数据解构部署了 Iterator 接口，就可以对它使用扩展运算符，将其转为数组。
...
yiled* 后面跟的是一个可遍历的解构，它会调用该结构的遍历器接口。
...
上面代码中，情况一输出文件的第一行以后，就会执行 return 方法，关闭这个文件；情况二 会在执行 return 方法关闭文件之后，再抛出错误。
...
如果要通过 for...of 循环，获取数组的索引， 可以借助数组实例的 entries 方法 和 keys 方法。

for...of 循环调用 遍历器接口，数组的遍历器接口只返回具有数字索引的属性。 这一点跟 for...in 循环也不一样。
...
- 数组的键名是数字，但是 for...in 循环是以字符串作为 键名 "0"、"1"、"2" 等等。
