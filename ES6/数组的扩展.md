...
const a1 = [1, 2];
// 写法一
const a2 = [...a1]; // [...a1] return a new array
// 写法二
const [...a2] = a1;
```
...
// TypeErrot: Can not spread non-iterable object.
let arr = [...arrayLike]
``` 
...
扩展运算符内部调用的是数据解构的 Iterator 接口，因此只要有 Iterator 接口的对象，都可以使用 扩展运算符， 比如 Map 结构。
...
``` js
const obj = {a: 1, b: 2};
let arr = [...obj]; // TypeError: Cannot spread nn-iterable object
```
---
...
上面代码中， querySelectorAll 方法返回的是一个类数组的对象，可以将这个对象转为真正的数组，再使用 filter 方法。
...
对于还没有部署该方法的浏览器，可以用 Array.prototype.slice 方法代替。
...
这实际上意味着，只要有一个原始的数据结构，你就快可以先对它的值进行处理，然后转成规范的数据结构，进而就可以使用数量众多的数组方法。
...
``` js
const contains = (() => 
  Array.prototype.includes ? (arr, value) => arr.includes(values)
...
数组的成员有时还是数组，Array.prototype.flat() 用于将嵌套的数组"拉平"，变成一维的数组。 该方法返回一个新数组，对原始数据没有影响。
...
如果不管有多少层嵌套，都要转成一堆数组，可以用 Infinity 关键字作为参数。
